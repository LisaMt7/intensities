...
`;
 ...
 `;
    ...
    if (o2.worker.changed) {
      ...
      sorted.forEach((count2, identifier) => {
        if (i2 < maxToVisualize) {
          ...
          const container = o2.containers[i2];
          const boxInfo = o2.worker.alphabetData.get(identifier);
          heatmap(container, o2.worker.frequencies, o2.worker.duration, boxInfo);
          ...
          const patternShapes = [];
          boxInfo.times.forEach((info2) => {
            const shape = {
              type: "rect",
              xref: "x",
              yref: "y",
              x0: info2.i * o2.worker.duration,
              y0: boxInfo.bin,
              x1: (info2.i + 1) * o2.worker.duration,
              y1: boxInfo.bin + o2.worker.freqWindow,
              opacity: 1,
              line: {
                width: 1,
                color: "gray"
          ...
          container.canvas.onclick = () => {
            if (!selected) {
              patternShapes.forEach((s2) => s2.line.color = "white");
              spectrogram.Plotly.relayout(spectrogram.div, { shapes });
              selected = true;
            } else {
              patternShapes.forEach((s2) => s2.line.color = "gray");
              spectrogram.Plotly.relayout(spectrogram.div, { shapes });
              selected = false;
            }
          };
  ...
  var features = [];
  var designTab = document.getElementById("design") ?? document.getElementById("graph");
  ...
  var plotData = (data2 = audio.fftData, which = dataSelect.element.value, how = transformation.element.value, thresh = threshold.element.value) => {
    return new Promise((resolve) => {
      if (data2[0]) {
        overlayDiv.innerHTML = `<h3>Plotting ${data2[0].length} FFT windows...</h3>`;
        overlay.open = true;
        setTimeout(() => {
          let plottedData;
          switch (which) {
...
`, event);
  ...
  var ripThroughFile = async (file2) => {
    const type = file2.type.split("/")[0];
    const alreadyPlotted = !!fileFFTs[file2.name];
    ...
    info2.worker.alphabetData.forEach((o2) => {
      ...
      const firstFreq = binToFreq(minFreqBin + o2.bin, hzPerBin);
      o2.frequencies = [firstFreq, binToFreq(minFreqBin + o2.bin + info2.worker.freqWindow, hzPerBin)];
    });
  ...
  var spawnStreamDisplay = (count2, o2 = {}) => {
    ...
    sourceRegistry[count2] = {
      ...
      video: o2.video,
      stream: o2.stream,
      spectrogram: new index$1.data.Spectrogram()
    };
